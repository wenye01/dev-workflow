import type { RecipeDefinition } from "../contracts/index.js";
import type { RecipeRegistry } from "../ports/index.js";

export class StaticRecipeRegistry implements RecipeRegistry {
  private readonly definitionsByKey = new Map<string, RecipeDefinition>();
  private readonly definitionsByRef = new Map<string, RecipeDefinition>();
  private readonly ambiguousRefs = new Set<string>();

  constructor(definitions: Iterable<RecipeDefinition> = []) {
    for (const definition of definitions) {
      this.add(definition);
    }
  }

  async resolve(ref: string, version?: string): Promise<RecipeDefinition | undefined> {
  ...
  private add(definition: RecipeDefinition): void {
    const key = registryKey(definition.ref, definition.version);
    if (this.definitionsByKey.has(key)) {
      throw new Error(`Duplicate recipe definition for ${definition.ref}@${definition.version}.`);
    }
