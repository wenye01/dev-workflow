...
func TestClaudeBuildArgs_ModesAndPermissions(t *testing.T) {
	...
	t.Run("new mode includes model when configured", func(t *testing.T) {
		cfg := &Config{Mode: "new", ClaudeModel: "opus-test"}
		got := backend.BuildArgs(cfg, "todo")
		want := []string{"-p", "--model", "opus-test", "--setting-sources", "", "--output-format", "stream-json", "--verbose", "todo"}
		if !reflect.DeepEqual(got, want) {
			t.Fatalf("got %v, want %v", got, want)
		}
	})
