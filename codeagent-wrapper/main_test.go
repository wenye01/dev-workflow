...
func TestBackendParseArgs_ModelFlags(t *testing.T) {
	...
	t.Setenv("CLAUDE_MODEL", "claude-env")
	...
	tests := []struct {
	...
	}{
		{
			...
			wantClaude:  "claude-env",
			wantGemini:  "gemini-env",
		},
		{
			name:        "claude model flag",
			args:        []string{"codeagent-wrapper", "--backend", "claude", "--claude-model", "opus-test", "task"},
			wantBackend: "claude",
			wantCodex:   "gpt-env",
			wantClaude:  "opus-test",
			wantGemini:  "gemini-env",
		},
		{
		...
		{
			name:        "generic model follows final claude backend",
			args:        []string{"codeagent-wrapper", "--model", "opus-test", "--backend", "claude", "task"},
			wantBackend: "claude",
			wantCodex:   "gpt-env",
			wantClaude:  "opus-test",
			wantGemini:  "gemini-env",
		...
		{
			name:    "missing claude model value",
			args:    []string{"codeagent-wrapper", "--claude-model"},
			wantErr: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cfg.Backend != tt.wantBackend || cfg.CodexModel != tt.wantCodex || cfg.ClaudeModel != tt.wantClaude || cfg.GeminiModel != tt.wantGemini {
				t.Fatalf("got backend=%q codex=%q claude=%q gemini=%q, want backend=%q codex=%q claude=%q gemini=%q",
					cfg.Backend, cfg.CodexModel, cfg.ClaudeModel, cfg.GeminiModel, tt.wantBackend, tt.wantCodex, tt.wantClaude, tt.wantGemini)
			}
		})
	}
}

func TestBackendParseArgs_WebUIDefaultsOff(t *testing.T) {
	oldWebUIMode := webUIMode
	oldLiteMode := liteMode
	t.Cleanup(func() {
		webUIMode = oldWebUIMode
	...
	if shouldStartWebServer() {
		t.Fatalf("web UI should be disabled by default")
	}

	os.Args = []string{"codeagent-wrapper", "--web-ui", "task"}
	if _, err := parseArgs(); err != nil {
		t.Fatalf("parseArgs() unexpected error: %v", err)
	}
	if !shouldStartWebServer() {
		t.Fatalf("web UI should be enabled by --web-ui")
	}

	os.Args = []string{"codeagent-wrapper", "--lite", "--web-ui", "task"}
	if _, err := parseArgs(); err != nil {
		t.Fatalf("parseArgs() unexpected error: %v", err)
	}
	if shouldStartWebServer() {
		t.Fatalf("lite mode should keep web UI disabled")
	}
}
...
func TestRunBuildCodexArgs_WithOutputSchema(t *testing.T) {
	const key = "CODEX_REQUIRE_APPROVAL"
	t.Cleanup(func() { os.Unsetenv(key) })
	os.Unsetenv(key)

	cfg := &Config{Mode: "new", WorkDir: "/test/dir", OutputSchemaPath: "/tmp/schema.json"}
	args := buildCodexArgs(cfg, "my task")
	expected := []string{
		"e",
		"--dangerously-bypass-approvals-and-sandbox",
...
func TestRunBuildCodexArgs_ResumeModeWithOutputSchema(t *testing.T) {
	const key = "CODEX_REQUIRE_APPROVAL"
	t.Cleanup(func() { os.Unsetenv(key) })
	os.Unsetenv(key)

	cfg := &Config{Mode: "resume", SessionID: "session-abc", OutputSchemaPath: "/tmp/schema.json"}
	args := buildCodexArgs(cfg, "-")
	expected := []string{
		"e",
		"--dangerously-bypass-approvals-and-sandbox",
