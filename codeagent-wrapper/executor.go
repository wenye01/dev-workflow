...
func buildCodexArgs(cfg *Config, targetArg string) []string {
	...
	if model := effectiveModel(cfg, "codex"); model != "" {
	...
	if isResume {
		resumeArgs := append(args, "--json", "resume")
		if schemaPath := strings.TrimSpace(cfg.OutputSchemaPath); schemaPath != "" {
			resumeArgs = append(resumeArgs, "--output-schema", schemaPath)
		}
		return append(resumeArgs, resumeSessionID, targetArg)
	}
...
func runCodexTaskWithContext(parentCtx context.Context, taskSpec TaskSpec, backend Backend, customArgs []string, useCustomArgs bool, silent bool, timeoutSec int) TaskResult {
	...
	// Start WebServer only when explicitly enabled. This avoids opening a
	// browser/SSE server during normal automation runs.
	var webSessionID string
	if shouldStartWebServer() && globalWebServer == nil {
...
func shouldStartWebServer() bool {
	return !liteMode && webUIMode
}
