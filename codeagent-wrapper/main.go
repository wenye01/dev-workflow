...
// Lite mode: reduce logging and post-message delay.
...
// Web UI/SSE is opt-in. It is useful for interactive debugging, but should not
// be enabled by default in automation because it starts a local server and opens
// a browser.
var webUIMode = os.Getenv("CODEAGENT_WEB_UI") == "true"
...
func run() (exitCode int) {
	...
	if cfg.Backend == "gemini" && cfg.Model != "" {
		envModel := strings.TrimSpace(os.Getenv("GEMINI_MODEL"))
		if envModel != "" && envModel == cfg.Model {
			logInfo(fmt.Sprintf("Gemini model from env: %s", cfg.Model))
		}
	}
	...
	if cfg.Model != "" {
		logInfo(fmt.Sprintf("Using %s model: %s", cfg.Backend, cfg.Model))
	}
	...
	if cfg.OutputSchemaPath != "" {
		...
		if backendSupportsNativeOutputSchema(cfg.Backend) {
			taskText = appendJSONOutputInstruction(taskText)
		} else {
	...
	if cfg.Backend == "mock" {
		startedAt := time.Now()
		result := runMockTask(cfg, outputSchema)
		durationMs := time.Since(startedAt).Milliseconds()
		if cfg.JSONOutput {
			payload := buildStepResult(result, mockArtifacts(cfg, result, outputSchema), durationMs)
			payload.Agent = cfg.Backend
...
Usage:
    ...
    %[1]s --lite "task" [workdir]     Lite mode (faster)
...
Options:
    ...
    --web-ui              Enable local browser Web UI/SSE output for debugging
    --backend <name>      Select backend (codex, gemini, claude)
    --model <name>        Specify model for selected backend (codex/gemini/claude)
    ...
    --claude-model <name> Specify Claude model (claude backend only)
    ...
    --output-schema <path> Load JSON schema from file for output guidance and artifact extraction

Environment Variables:
    CODEX_TIMEOUT              Timeout in milliseconds (default: 7200000)
    CODEX_MODEL                Codex model passed as -m/--model
    CLAUDE_MODEL               Claude model passed as --model
    GEMINI_MODEL               Gemini model passed as -m
    ...
    CODEAGENT_WEB_UI           Enable local browser Web UI/SSE output (default: false)
